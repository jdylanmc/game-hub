...
function validateLintIntegrationPolicy({
  ...
  for (const [fragment, expectedCount] of [
    ['node-version: 24.13.0', 2],
    ['run: corepack enable', 2],
    ['run: yarn install --immutable 2>&1 | tee continuous-integration-evidence/install.log', 1],
    ['run: yarn lint 2>&1 | tee continuous-integration-evidence/lint.log', 1],
    ['run: yarn test:ci-fail-closed 2>&1 | tee continuous-integration-evidence/fail-closed.log', 1],
    ['            continuous-integration-evidence/', 1],
  ]) {
    if (occurrences(workflowSource, fragment) !== expectedCount) {
      lintViolations.push(`Continuous integration lint contract must contain exactly ${expectedCount}: ${fragment}`);
    }
  }
